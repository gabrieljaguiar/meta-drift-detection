...
class AdaptiveWindowing:
    ...
    def __init__(
        self, delta=0.002, clock=32, max_buckets=5, min_window_length=5, grace_period=10
    ):
        self.delta = delta
    ...
    def _insert_element(self, value: float, variance: float):
    ...
    def _delete_element(self):
        ...
        incremental_variance = v + n * self.width * (mu - mu_window) * (
            mu - mu_window
        ) / (n + self.width)
        self.variance -= incremental_variance
                ...
                for idx in range(len(self.bucket_deque) - 1, -1, -1):
                    ...
                    for k in range(bucket.current_idx - 1):
                        ...
                        if n0 > 0.0:
                            ...
                            v0 += bucket.get_variance_at(k) + n0 * n2 * (mu0 - mu2) * (
                                mu0 - mu2
                            ) / (n0 + n2)

                        if n1 > 0.0:
                            mu1 = u1 / n1  # mean of window 1
                            v1 -= bucket.get_variance_at(k) + n1 * n2 * (mu1 - mu2) * (
                                mu1 - mu2
                            ) / (n1 + n2)
    ...
    def _evaluate_cut(self, n0: float, n1: float, delta_mean: float, delta: float):
        ...
        epsilon = (
            sqrt(2 * m_recip * self.variance_in_window * delta_prime)
...
class Bucket:
    ...
    def clear_at(self, index: int):
        self.set_total_at(0.0, index)
        self.set_variance_at(0.0, index)

    def insert_data(self, value: float, variance: float):
    ...
    def compress(self, n_elements: int):
    ...
    def get_total_at(self, index: int):
        return self.total_array[index]

    def get_variance_at(self, index: int):
        return self.variance_array[index]

    def set_total_at(self, value: float, index: int):
        self.total_array[index] = value

    def set_variance_at(self, value: float, index: int):
